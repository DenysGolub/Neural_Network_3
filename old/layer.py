...
class Layer:
    ...
    def back_prop_output(self, expected):
        self.error = LossFunctions.calculate_loss(expected, self.outputs, self.loss_function)
        self.error_gradient = self.error * np.array(Activations.activate_back(self.outputs, self.activation))
